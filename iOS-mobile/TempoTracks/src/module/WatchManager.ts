...
export const IS_WATCH_ENABLED = true;

const watchEventEmitter = new NativeEventEmitter(
  NativeModules.WatchManagerEmitter
);
...
export const sendSongsToWatch = () => {
  ...
  useEffect(() => {
    if (isSuccess && songs) {
      ...
      WatchManager.sendSongs(songsJson)
        .then(() => {
...
export const sendWorkoutTemplatesToWatch = () => {
  const { data: templates, isSuccess } = useGetWorkoutTemplates(
