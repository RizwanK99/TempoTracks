...
serve(async (req) => {
  ...
  if (acessTokenError) {
    return new Response(JSON.stringify({ error: acessTokenError }), {
      headers: { 'Content-Type': 'application/json' },
    });
  }

  // get list of spotify song ids and other metadata based on song titles and artists
  const songsSpotifyMetadata = await Promise.all(
    songs.map(async (song: any) => {
      const { title, artist } = song;
      const trackRes = await fetch(
        `https://api.spotify.com/v1/search?q=${title} ${artist}&type=track`,
        {
          headers: {
      ...
      return {
        artists: topTrack.artists.map((artist: any) => artist.name),
        title: topTrack.name,
  ...
  const songsWithAudioFeatures = songsSpotifyMetadata.map((song) => {
    const audioFeature = audioFeatures.audio_features.find(
      (audioFeature: any) => audioFeature.id === song.spotifyId
    );
    return _.omit(
      {
        ...song,
  ...
  if (insertSongsError) {
    return new Response(JSON.stringify({ error: insertSongsError }), {
      headers: { 'Content-Type': 'application/json' },
    });
  }

  return new Response(JSON.stringify({ songsWithAudioFeatures }), {
    headers: { 'Content-Type': 'application/json' },
  });
});
